...
-spec register(term(), pid()) ->
    registered
    | {error,
        {could_not_register, Reason :: term()} | {already_registered, pid()} | timeout | term()}.
register(Id, Value) ->
...
handle_event(enter, _, commit, #state{id = Id, nodes = Nodes, term = Term, pid = Pid} = State) ->
    ...
    {keep_state,
        set_promises(
            [commit(Node, Id, Term, Data) || Node <- Nodes],
...
handle_event(
    info,
    #promise_reply{result = {nack, {Server, not_found}}} = Reply,
    monitoring,
    ...
    NewResult =
        case pes_promise:await(repair(Server, Id, not_found, Term, {Pid, self(), Now})) of
            ack -> ack;
            _ -> nack
        end,
...
handle_event(
    info,
    #promise_reply{result = {nack, {Server, OldTerm}}} = Reply,
    monitoring,
    #state{id = Id, term = Term, pid = Pid, last_timestamp = Now} = State
) ->
...
handle_event({call, From}, {update, _}, StateName, _State) when
    StateName =/= monitoring andalso StateName =/= registered
->
...
handle_read(Ref, Reply, State) ->
    case evaluate_response(Ref, Reply, State) of
            ...
            {keep_state, NewState};
        {
            ...
            {HTerm, Count} = maps:fold(
                fun
                    (not_found, Count, {HighestTerm, Acc}) ->
...
wait(Id, Pid, Rand) ->
    ...
    BaseTime = erlang:phash2({node(), Pid, Id}, 1 bsl 4),
    timer:sleep(BaseTime + rand:uniform(Rand)).

evaluate_replies(Replies, Majority) ->
    maps:fold(
        fun
            (Result, VoteCount, _Acc) when VoteCount >= Majority ->
                Result;
...
is_term_repairable(Server, Id, Term) ->
    case pes_promise:await(pes_server_sup:read(Server, Id)) of
        {ok, ReadTerm, {OldPid, _OldGuardPid, TimeStamp}} when
        ...
        % tombstone able to repair
        {ok, ReadTerm, undefined} when ReadTerm =:= Term ->
            true;
        not_found ->
            true;
        % error or the term has changed
        _Else ->
            false
    end.
